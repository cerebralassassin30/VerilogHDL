...
module demux_1x2_dfmdl_tb;
  reg EN,I,S;
  wire [1:0] Y;
  initial begin
    $monitor("simtime = %0t, Y = %b, EN = %b, I = %b, S = %b", $time, Y,EN,I,S );
  end
  demux_1x2_dfmdl dut(.y(Y), .en(EN), .i(I), .s(S));
...
// EN = 1 and I = 1 → The select line S decides which output gets the 1.
  ...
  initial begin
    ...
    // TESTCASE 1 
     ...
     EN = 1'b0; I = 1'b0; S = 1'b0; // as the enable input is 0, the DEMUX is disabled; therefore, the input data (I) and select line (S) are ignored. Hence, EXPECTED OUTPUT Y = 2'b00
    
    // TESTCASE 2
    
    #10   EN = 1'b0; I = 1'b0; S = 1'b1; // as the enable input is 0, the DEMUX is disabled; therefore, the input data (I) and select line (S) are ignored. Hence, EXPECTED OUTPUT Y = 2'b00
    
    // TESTCASE 3
    
    #10   EN = 1'b0; I = 1'b1; S = 1'b0; // as the enable input is 0, the DEMUX is disabled; therefore, the input data (I) and select line (S) are ignored. Hence, EXPECTED OUTPUT Y = 2'b00
    
    // TESTCASE 4
    
    #10  EN = 1'b0; I = 1'b1; S = 1'b1; // as the enable input is 0, the DEMUX is disabled; therefore, the input data (I) and select line (S) are ignored. Hence, EXPECTED OUTPUT Y = 2'b00
